...
export default function LancePayment() {
  const [cardNumber, setCardNumber] = useState("4200000000000000");
  ...
  const [cardName, setCardName] = useState("Jeena Doe");
  const [cardCity, setCardCity] = useState("San Francisco");
  const [cardCountry, setCardCountry] = useState("US");
  const [cardLine1, setCardLine1] = useState("123 Main St");
  ...
  return (
    <div className="flex flex-col gap-8">
      <h1 className="text-2xl font-bold underline">Lance Payment</h1>
        ...
        <div className="flex w-full flex-col gap-4">
          ...
          <div className="rounded-md bg-lime-500 px-12 py-4 text-center text-black">
            {match(paymentMutation)
              ...
              .with({ status: "idle" }, () => (
                <p className="text-lg">Idle...</p>
              ))
              .with({ status: "success" }, ({ data }) => (
                <p className="text-lg">Success! ID: {data!.id}</p>
              ))
      ...
      <div className="rounded-md bg-indigo-500 px-12 py-4 text-center text-black overflow-y-auto text-xs">
            {match(paymentMutation)
              .with({ status: "loading" }, () => (
                <p className="text-lg">Contacting Circle for Payment and getting data...</p>
              ))
              .with({ status: "idle" }, () => (
                <p className="text-lg">Idle...</p>
              ))
